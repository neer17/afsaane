...
import { ACTIVE_COUNTRIES, USER_ROLES } from "@/utils/constants";
			...
			try {
				...
				if (savedState) {
					setIsVerificationCodeSent(
						savedState.isVerificationCodeSent,
					);
					setIsVerificationCodeVerified(
						savedState.isVerificationCodeVerified,
					);
		...
		const saveState = async () => {
			if (formRef.current) {
				...
				const useDifferentBilling =
					formRef.current.getUseDifferentBilling();
		...
		if (otpExpiryTime && timeRemaining > 0) {
			interval = setInterval(() => {
				...
				const remaining = Math.max(
					0,
					Math.ceil((otpExpiryTime - now) / 1000),
				);
					...
					<Grid overflow="hidden">
						<Grid.Col span={{ base: 12, lg: 7 }}>
							<CheckoutForm
								...
								} // TODO: hack to not send otp, SHOULD BE REMOVED
									...
									<Stack>
										<Group justify="space-between">
											<TextInput
												...
												onChange={
													handleDiscountCouponInputChange
												}
											/>
											<Button
